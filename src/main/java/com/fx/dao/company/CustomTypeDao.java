...
@Repository
public class CustomTypeDao extends ZBaseDaoImpl<CustomType, Long> {
	/** 日志记录 */
	 ...
	 * @return Page<T>	分页数据
	 */
	public Page<CustomType> findCustomType(ReqSrc reqsrc, String page, String rows,String unitNum, String typeName,
			String sTime, String eTime) {
		String logtxt = U.log(log, "获取-单位银行帐金额类型-分页列表", reqsrc);
		
		Page<CustomType> pd = new Page<CustomType>();
		...
		try{
			if(ReqSrc.PC_BACK != reqsrc && ReqSrc.PC_COMPANY != reqsrc) {// 没有查询出数据
				...
				if(StringUtils.isNotEmpty(typeName)){
					filts.add(new Filtration(MatchType.LIKE, typeName, "typeName"));
				}
