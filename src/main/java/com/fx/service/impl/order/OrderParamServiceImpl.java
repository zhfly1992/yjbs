...
import com.fx.service.order.OrderParamService;

@Service
@Transactional
public class OrderParamServiceImpl extends BaseServiceImpl<OrderParam, Long> implements OrderParamService {
	/** 日志记录 */
	...
	@Override
	public ZBaseDaoImpl<OrderParam, Long> getDao() {
		return orderParamDao;
	}
	@Override
	public Map<String, Object> addOrderParam(ReqSrc reqsrc, String uname,String phone, String unitNum, String backRelNum,
			String num, String isShuttle, String spoint, String epoint, String wpoints, String gotime, String flyOrDownTime) {
		
		String logtxt = U.log(log, "添加-单程接送-临时订单数据", reqsrc);
		...
		try {
			...
			if(fg) {
				OrderParam op=orderParamDao.addOrderParam(backRelNum, 1, unitNum, serviceType, uname, phone, 1, _isShuttle, 
						spoint, epoint, num, wpoints, MathUtils.div(ucp2ucp.getDistance(), 1000, 1), ucp2ucp.getTimeCons()/60, stime, etime, fdtime, null);
				if(op!=null) {
					map.put("id", op.getId());
					U.setPut(map, 1, "添加成功");
				}else {
